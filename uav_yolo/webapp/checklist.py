...
DEFAULT_ITEMS: list[tuple[str, str]] = [
    ...
    ("PX4 / 雲台參數", "定翼：NAV_LOITER_RAD 設為需求繞行半徑（LR24 模式半徑由此決定）"),
    ("整合 NYCU offboard（lr24 模式才需要）", "指令後端設定正確：direct（四軸首驗）或 lr24（整合）"),
    ("整合 NYCU offboard（lr24 模式才需要）", "SiK MAVLink 電台（讀 pose，接 TELEM1）與 LR24-F（送 GOTO）為兩條不同埠"),
    ("整合 NYCU offboard（lr24 模式才需要）", "Pixhawk TELEM2：MAV_1_CONFIG 停用、UXRCE_DDS_CFG=TELEM2、SER_TEL2_BAUD 一致"),
    ("整合 NYCU offboard（lr24 模式才需要）", "機上 Micro-XRCE-DDS Agent 已跑、ros2 topic list 看得到 /fmu/ topics"),
    ("整合 NYCU offboard（lr24 模式才需要）", "companion 已啟 serial_gps_goto.launch.py；四軸須套多旋翼 patch 並過 SITL"),
    ("整合 NYCU offboard（lr24 模式才需要）", "send_lr24_command.py STATUS 回 ready_for_goto=true 才可開導引"),
